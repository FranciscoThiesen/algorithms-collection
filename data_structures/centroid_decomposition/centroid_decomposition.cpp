...
struct centroid_decomposition
{
	...
	int centroid(int root, const vector<vi>& g, const deque<bool>& dead)
	{
		...
		function<int(int, int)> dfs = [&](int loc, int par) {
			for (auto& v: g[loc])
			{
				if (v != par && !dead[v])
				{
					if (subtree_sz[v] > n / 2) { return dfs(v, loc); }
				}
			}
			return loc;
		};
	...
	long long doit(const vector<vi>& g, const int k)
	{
		...
		function<map<int, long long>(int, int)> process_subtree =
			[&](int root, int parent) {
				map<int, long long> path_count;

				function<void(int, int, int)> explore =
					[&](int root, int parent, int dist) {
		...
		function<void(int)> rec = [&](int start) {
			...
			for (const int& nei: g[c])
			{
				if (!dead[nei])
				{
					...
					TRACE(cout << "estou processando o filho do 0 = " << nei
							   << endl;)
					TRACE(for (const auto& v
							   : fuark) cout
							  << v.first << " " << v.second << endl;)
					for (const auto& val: fuark)
					{
						int complemento = k - val.first;
						TRACE(cout << " val = " << val.first
								   << " comp = " << complemento << endl;)
						ans = (ans + (val.second * general_count[complemento]));
						ans = (ans % mod);
					}
					TRACE(cout << " apos processar " << nei << " ans = " << ans
							   << endl;)
					for (const auto& val: fuark)
						general_count[val.first] += val.second;
				}
			}
			for (auto u: g[c])
				if (!dead[u]) rec(u);
