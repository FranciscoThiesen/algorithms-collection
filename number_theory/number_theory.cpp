...
template< typename T>
T mod(T x, T m) { return x % m + (x < 0) ? m : 0; }

template< typename T>
T solve_mod(T a, T b, T m)
{
    ...
    if(b % d) return -1;
    else return mod<T>(bezout.first * (b / d), m);
}
...
void linear_sieve()
{
    for(int i = 2; i < MAXN; ++i)
    {
        ...
        for(int j = 0; j < (int) primes.size() && primes[j] <= min_f[i] && i*primes[j] < MAXN; ++j)
